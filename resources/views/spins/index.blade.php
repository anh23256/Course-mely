...
@push('page-css')
    <style>
        ...
        .col-70 { flex: 0 0 70%; max-width: 70%; padding: 0 10px; }
        .col-30 { flex: 0 0 30%; max-width: 30%; padding: 0 10px; }
...
@section('content')
    <div class="container-fluid">
                ...
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    ...
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active"><a href="{{ route('admin.transactions.index') }}">{{ $title ?? '' }}</a></li>
                        </ol>
                    </div>
                </div>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addSpinConfigModal">
                                ...
                                @foreach ($spinConfigs as $config)
                                    <tr data-id="{{ $config->id }}" data-type="spin_config">
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('admin.spins.deleteSpinConfig', $config->id) }}" method="POST" class="d-inline delete-spin-config-form">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" class="btn btn-danger btn-sm delete-spin-config" onclick="return confirm('Bạn chắc chắn muốn xóa ô quà này?')">Xóa</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
                            ...
                            <tbody>
                                @foreach ($giftsAll as $gift)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($gift->image_url)
                                                <img src="{{ $gift->image_url }}" alt="{{ $gift->name }}" style="max-width: 50px;">
                                            @else
                                                Không có ảnh
                                            @endif
                                        </td>
        ...
        <div class="modal fade" id="addSpinConfigModal" tabindex="-1" aria-labelledby="addSpinConfigModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('admin.spins.spin-config.store') }}" method="POST" class="row g-3">
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" name="type" value="{{ old('type') }}" class="form-control" placeholder="Loại (ví dụ: coupon, message)" required>
                            </div>
                            <div class="col-md-12">
                                <label for="name" class="form-label">Tên</label>
                                <input type="text" name="name" value="{{ old('name') }}" class="form-control" placeholder="Tên ô quà" required>
                            </div>
                            <div class="col-md-12">
                                <label for="probability" class="form-label">Tỷ lệ (%)</label>
                                <input type="number" name="probability" value="{{ old('probability') }}" class="form-control" placeholder="Tỷ lệ (%)" step="0.01" min="0" max="100" required>
                        ...
                        <form method="GET" action="{{ route('admin.spins.index') }}" class="filter-form row g-3">
                            <div class="col-md-4">
                                ...
                                <select name="filter_type" id="filter_type" class="form-control" onchange="this.form.submit()">
                                    <option value="day" {{ request('filter_type', 'day') === 'day' ? 'selected' : '' }}>Theo ngày</option>
                            ...
                            @if (request('filter_type', 'day') === 'day')
                                <div class="col-md-4">
                                    ...
                                    <input type="date" name="filter_date" value="{{ request('filter_date', now()->format('Y-m-d')) }}" class="form-control" onchange="this.form.submit()">
                                </div>
                            @elseif (request('filter_type') === 'month')
                                <div class="col-md-4">
                                    <label for="filter_month" class="form-label">Chọn tháng</label>
                                    <input type="month" name="filter_month" value="{{ request('filter_month', now()->format('Y-m')) }}" class="form-control" onchange="this.form.submit()">
                                </div>
